...
test_that("import periods CSV  works", {
  ...
  excepted <- as.data.frame(quiet_read_csv(file="./csv/import_periods_csv/after.csv")$result)
...
test_that("import periods CSV works when import twice", {
  ...
  quiet_read_csv <- purrr::quietly(readr::read_csv)
  excepted <- as.data.frame(quiet_read_csv(file="./csv/import_periods_csv/after2.csv")$result)
