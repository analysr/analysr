test_that("check_tables_integrity works", {
  ...
  setup_new_env()
  analysr_env$periods <- data.frame(stat_unit = character(0), hash = integer(0),
                      begin = numeric(0), end = numeric(0), tag = character(0))
  expect_equal(quiet_check()$result, FALSE)


  # bad labels on measures table
  setup_new_env()
  analysr_env$measures <- data.frame(hash = integer(0), stat_unit = character(0),
                      begin = numeric(0), end = numeric(0), tag = character(0))
  expect_equal(quiet_check()$result, FALSE)
